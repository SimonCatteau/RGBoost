...
def execute_rule(rule, feature_data, target_data, prediction_update, g_mean):
    ...
    g_vector_update = vectorized_update(target_data, prediction_update)
    return prediction_update, g_vector_update


def update_g_vector(target_data, target_prediction):
    """
    update the g_value for 1 example
    :param target_data:
    ...
    return -(2 * target_data) / (1 + np.exp(2 * target_data * target_prediction))
